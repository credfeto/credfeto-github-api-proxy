/**
 * Rewrites absolute api.github.com URLs embedded in response bodies and
 * headers so that a client following one of them lands back on this proxy
 * instead of bypassing it and hitting real GitHub directly with credentials
 * that are only valid against the proxy (issue #65).
 *
 * html_url fields (github.com, the web UI) are untouched by construction:
 ...
 * URL in its string values, and re-serialises. Mirrors meta.ts's
 * injectInstalledVersion: malformed JSON is returned unchanged, and the
